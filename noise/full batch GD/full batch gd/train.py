...
def run_training(args, data):
    ...
    # Combine all training data into a single batch
    X_full, Y_full = [], []
...
# Configure optimizer for pure gradient descent
    optimizer = optim.SGD(
        model.parameters(),
        lr=args.lr 
    ...
    print('Start Training with Full Batch Gradient Descent...')
    min_loss = np.inf
    train_losses = []
    val_losses = []
    train_violations = []
    ...
    for epoch in range(args.epochs):
        ...
        if args.model == 'AugLagNN':
            ...
            mu_k = (epoch + 1) * args.mu
            mse_loss = optimizer_step_full_batch(model, optimizer, loss_func, X_full, Y_full, args, data, lambda_k, mu_k)
            pred_diff = conservation_step(model, X_full, data, args)
                # update
            with torch.no_grad():
            ...
            mse_loss = optimizer_step_full_batch(model, optimizer, loss_func, X_full, Y_full, args, data)
            pred_diff = conservation_step(model, X_full, data, args)
            train_loss = mse_loss
            train_violation += torch.abs(pred_diff.view(-1)).mean()
...
def optimizer_step_full_batch(model, optimizer, loss_func, X, Y, args, data, lambda_k=None, mu_k=None):
