    ...
    try:
        ...
        with torch.no_grad():
            for X, Y in data["test_loader"]:
                ...
                violation_rxn += torch.abs(pred_diff[:, 0]).mean().item()
                violation_mb += torch.abs(pred_diff[:, 1]).mean().item()
                

        rmse_total = np.sqrt(rmse_total / len(data["test_loader"]))
        violation = (violation / len(data["test_loader"])).item()
        violation_rxn = violation_rxn / len(data["test_loader"])
        violation_mb = violation_mb / len(data["test_loader"])
        

        scores.update({
            "rmse_total": float(rmse_total),
            "violation": float(violation),
            "violation_rxn": float(violation_rxn),
            "violation_mb": float(violation_mb),
        })
        ...
        nan_scores = {"rmse_total": float("nan"), "violation": float("nan"), "violation_rxn": float("nan"), "violation_mb": float("nan")}
