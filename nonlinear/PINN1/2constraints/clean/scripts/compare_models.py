...
def main():
    ...
    parser.add_argument("--mu_rxn", type=float, default=0.05)
    parser.add_argument("--mu_mb", type=float, default=0.01)
    ...
    run_command([
        sys.executable, "-m", "src.linearization",
        "--nC_regions", "30",
    ])
    ...
    model_settings = [
        ...
        {
            "model": "PINN",
            "model_id": f"PINN_rxn{args.mu_rxn:g}_mb{args.mu_mb:g}_n{args.n_points}",
            "mu_rxn": args.mu_rxn,
            "mu_mb": args.mu_mb,
        },
        {
            "model": "KKThPINN",
            "model_id": f"KKThPINN_n{args.n_points}_seg30_2constraints",
            "mu_rxn": 0.0,
            "mu_mb": 0.0,
        },
]
    ...
    for setting in model_settings:
        ...
        for run in range(args.repeats):
            ...
            row = {
                ...
                "inference_violation_rxn": float(report["violation_rxn"]),
                "inference_violation_mb": float(report["violation_mb"]),
            }
    ...
    summary = (
        ...
        .agg(
            ...
            inference_violation_rxn_mean=("inference_violation_rxn", "mean"),
            inference_violation_rxn_std=("inference_violation_rxn", "std"),

            inference_violation_mb_mean=("inference_violation_mb", "mean"),
            inference_violation_mb_std=("inference_violation_mb", "std"),
        )
    )
