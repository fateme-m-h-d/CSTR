"""External PL-KKT-hPINN-style timing wrapper for KKT-HardNet."""
...
def parse_args():
    ...
    p.add_argument("--output", default="results/kkt_pl_style_benchmark.csv")
    return p.parse_args()
...
def main():
    ...
    for run in range(1, args.runs + 1):
        print(f"\n{'=' * 70}\nKKT-HARDNET RUN {run}/{args.runs}\n{'=' * 70}")

        train_cmd = [
            sys.executable,
            "run_kkthardnet_cstr.py",
            "--job",
            "train",
            "--data",
            args.data,
        ...
        # Same external training-time boundary as PL experiment2.py.
        ...
        metrics_path = base_dir / "results" / f"kkt_run_{run}_metrics.json"
        ...
        row = {
            ...
            **{
                ...
                if k not in {"run", "evaluation_time_sec"}
            },
        }
        rows.append(row)
