...
def load_region_edges(npz_path="region_edges.npz"):
    ...
    return edges["T_edges"].astype(float), edges["C_edges"].astype(float)
...
def LoadData(args):
    ...
    C_edges = C_edges_raw / scaler.scale_[1]
    ...
    assert n_regions == len(A_list), (
        f"{n_regions=} but {len(A_list)=}. Fix edges or ABb_matrices.csv."
    )
...
def nonlinear_residuals(X, pred, scaler):
    """Return signed reaction and mass-balance residuals in physical units."""
    scale = torch.as_tensor(
        scaler.scale_, dtype=pred.dtype, device=pred.device
    )

    # data.csv column order: T, Cao, Ca, Cb, Cc
    T = X[:, 0] * scale[0]
    Cao = X[:, 1] * scale[1]
    Ca = pred[:, 0] * scale[2]
    Cb = pred[:, 1] * scale[3]
    ...
    tau = 10.0
    kf = 1e13 * torch.exp(-90000.0 / (8.314 * T))
    kr = 1e11 * torch.exp(-80000.0 / (8.314 * T))

    r_rxn = Cao - Ca - kf * Ca * Cb**2 * tau + kr * Cc * tau
    r_mb = Cc - Cao + Ca - 2.0 + Cb
...
class PINNLoss(nn.Module):
    ...
    def forward(self, X, pred, target):
        ...
        residual = nonlinear_residuals(X, pred, self.scaler)
        pinn_loss = (
            self.mu_rxn * torch.mean(residual[:, 0] ** 2)
            + self.mu_mb * torch.mean(residual[:, 1] ** 2)
        )
...
def compute_violation_original_nonlinear(
    ...
    with torch.no_grad():
        return nonlinear_residuals(
            X_scaled, Ypred_scaled, scaler
        ).abs().to(device)
