...
from src.config import N_REPEATS, SEGMENT_SCENARIOS
...
def summarize_experiment_file(nC):
    ...
    row = {"nC_regions": nC, "num_regions": nC, "partition": "taylor_intervals"}
    pooled_nn = {}
...
def main():
    # Adaptive centers are solved offline, preserving the original NN dataset.
    if not (BASE_DIR / "data.csv").is_file():
        raise FileNotFoundError("Copy the original 1D-V/data.csv into this folder first")
    ...
    for nC in SEGMENT_SCENARIOS:
        print(f"Running adaptive scenario n_regions={nC}")
        shutil.copy2(BASE_DIR / "data_fixed.csv", BASE_DIR / "data.csv")

        run_cmd([PYTHON_EXE, "-m", "src.linearization", "--n_regions", str(nC)])
        run_cmd([PYTHON_EXE, "-m", "diagnostics.linearization_accuracy"])

        env = os.environ.copy()
        env["SCENARIO_ID"] = f"adaptive_1d_{nC}"
