...
from models import NN, NNOPT, get_rectangle_index

device = "cpu"


def load_region_bounds(npz_path="region_edges.npz"):
    with np.load(npz_path) as artifact:
        if "region_bounds" not in artifact.files:
            raise ValueError(
                f"{npz_path} has no adaptive region_bounds. "
                "Run python -m src.linearization --n_regions 33 first."
            )
        bounds = artifact["region_bounds"].astype(float)
    if bounds.ndim != 2 or bounds.shape[1] != 4 or len(bounds) == 0:
        raise ValueError("region_bounds must have nonempty shape (n_regions, 4)")
    if not np.isfinite(bounds).all() or np.any(bounds[:, [1, 3]] <= bounds[:, [0, 2]]):
        raise ValueError("Each rectangle must have finite, strictly increasing bounds")
    return bounds
...
def LoadData(args):
    ...
    region_bounds = load_region_bounds()
    region_bounds[:, 0:2] /= scaler.scale_[0]
    region_bounds[:, 2:4] /= scaler.scale_[1]
    n_regions = len(region_bounds)
    ...
    assert n_regions == len(A_list), (
        f"{n_regions=} but {len(A_list)=}. Fix region_bounds or ABb_matrices.csv."
    )
...
def load_ABb_from_csv(csv_path="ABb_matrices.csv"):
    ...
    for _, region in frame.groupby("region_id", sort=True):
        ...
        b_list.append(torch.tensor(
            region["b"].to_numpy(dtype=float), dtype=torch.float32
        ))
...
def get_violation(args, data, X, pred):
    region = get_rectangle_index(X, data["region_bounds"])

    A = torch.stack(data["A_list"]).to(
        dtype=X.dtype,
        device=X.device,
    )

    B = torch.stack(data["B_list"]).to(
        dtype=X.dtype,
        device=X.device,
    )
