...
# === Configuration ===
base_dir = "C:/Users/Fateme/Desktop/Research/CSTR/nonlinear/firsthalf/lessnodes/25/150"
...
def run_main(model_name, job):
    ...
    if job == 'train':
        train_error, val_error = extract_last_epoch_error(result.stdout)
...
def extract_last_epoch_error(output):
    """
    Extract 'loss_train' and 'loss_val' from the last line in the output that starts with 'epoch:'.
    The output lines are expected to look like:
      epoch: 00050 loss_train: 0.0000 loss_val: 0.0000 violation_train: 0.0000 violation_val: 0.0000
    """
    lines = output.splitlines()
        ...
        if line.startswith("epoch:"):
            ...
            while i < len(parts):
                if parts[i].endswith(":"):
                    ...
                    i += 1
            try:
                loss_train = float(data["loss_train"])
                loss_val = float(data["loss_val"])
...
def run_model_experiments(model_name, num_iterations):
    ...
    for i in range(num_iterations):
        ...
        # Training run
        train_error, val_error = run_main(model_name, "train")
    ...
    return training_errors, validation_errors, experiment_errors
...
def get_predictions(model_name):
    ...
    input_dim = 1
    hidden_dim = 25 #changed 
    hidden_num = 2
    z0_dim = 3

    # Choose model path and parameters based on model_name
    if model_name == "NN":
        model_path = "C:/Users/Fateme/Desktop/Research/CSTR/nonlinear/firsthalf/lessnodes/25/150/new1/models/cstr/NN/0.2/MODELID_0.2_0.pth"
        model_type = "NN"
        A, B, b = None, None, None
    elif model_name == "KKThPINN":
        model_path = "C:/Users/Fateme/Desktop/Research/CSTR/nonlinear/firsthalf/lessnodes/25/150/new1/models/cstr/KKThPINN/0.2/MODELID_0.2_0.pth"
    ...
    new_temperatures = np.linspace(280, 360, 150)  # 300 points
...
def main():
    ...
    nn_training_errors, nn_validation_errors, nn_experiment_errors = run_model_experiments("NN", num_iterations)
    ...
    kkt_training_errors, kkt_validation_errors, kkt_experiment_errors = run_model_experiments("KKThPINN", num_iterations)
    ...
    training_df = pd.DataFrame({
        ...
        "NN_Validation_Error": nn_validation_errors,
        "KKThPINN_Training_Error": kkt_training_errors,
        "KKThPINN_Validation_Error": kkt_validation_errors
    })
