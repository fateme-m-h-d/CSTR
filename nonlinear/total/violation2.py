...
A_scaled = [A.float().to(DEVICE) for A in A_scaled]
B_scaled = [B.float().to(DEVICE) for B in B_scaled]
b_scaled = [b.float().to(DEVICE) for b in b_scaled]
...
T_query = np.array([300, 340, 360, 400, 500], dtype=float).reshape(-1, 1)

# 1) Scale T the same way you did for the grid
dummy_q = np.zeros((len(T_query), 4), dtype=float)
dummy_q[:, 0] = T_query[:, 0]
...
Tq_scaled_t = torch.tensor(Tq_scaled, dtype=torch.float32, device=DEVICE)
...
pack = np.zeros((len(T_query), 4), dtype=float)
...
dummy = np.zeros((len(T_unscaled), 4), dtype=float)
dummy[:,0] = T_unscaled[:,0]  # T column only, others zeros
X_scaled_np = scaler.transform(dummy)[:, :1]   # take first col
X_scaled = torch.tensor(X_scaled_np, dtype=torch.float32, device=DEVICE)  # (N,1)
...
def compute_violation_piecewise(T_vals, X_s, Z_s, Y, A_s_list, B_s_list, b_s_list,
    ...
    for idx_region, ((lo, hi), As, Bs, bs, Ar, Br, br) in enumerate(zip(
        ...
        # unscaled
        # build tensors
        X_raw_t = torch.tensor(T_raw_vals.reshape(-1,1), dtype=torch.float32, device=DEVICE)
        Z_raw_t = torch.tensor(Z_raw_vals, dtype=torch.float32, device=DEVICE)
        v_r_full = (Ar @ X_raw_t.T + Br @ Z_raw_t.T - br.view(-1,1)).cpu().numpy().flatten()
...
for i, v in enumerate(v_scaled_list):
    plt.plot(T_unscaled, v, label=f"Region {i+1}: {RANGES[i][0]}–{RANGES[i][1]} K")
