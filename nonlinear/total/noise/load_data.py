...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        A = A_list[-1]; B = B_list[-1]; b = b_list[-1]
        
    # if model.__class__.__name__ == "NNOPT":
    #     model.rebind_kkt(A_list, B_list, b_list)  # make sure KKT layers match current scaled A,B,b   

    T_eval = np.linspace(280, 600.0, 500)
    X4 = np.column_stack([T_eval, np.zeros_like(T_eval), np.zeros_like(T_eval), np.zeros_like(T_eval)])  # (N,4)
    ...
    A_s = A_list[-1].detach().cpu().numpy()     # (1,1)
    B_s = B_list[-1].detach().cpu().numpy()     # (1,3)
    b_s = b_list[-1].detach().cpu().numpy().reshape(1,)  # (1,)
    ...
    Z_raw = raw[:, 1:4]         # (N,3)   raw (Ca, Cb, Cc)
    ...
    plt.figure()
    plt.scatter(T_eval, np.abs(r_scaled), label="scaled residual")
    # plt.semilogy(T_eval, np.abs(r_raw), ".", label="raw residual")
    plt.xlabel("Temperature (K)")
    plt.ylabel("Violation |A·x + B·z − b|")
    plt.yscale('log')
    plt.title("KKT residual (last segment) 503–600 K")
    plt.legend()
