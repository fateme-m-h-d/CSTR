...
def LoadData(args):
    ...
    params = {'batch_size': args.batch_size,
              'shuffle': True}
    # params_train = {'batch_size': args.batch_size, 'shuffle': True}
    # params_eval  = {'batch_size': args.batch_size, 'shuffle': False}
...
def load_ABb_from_csv(csv_path="ABb_matrices.csv"):
    """
    Load one A, B, b block per region.

    For the 2-input, 2-constraint case:
        X = [T, Cao]
        Y = [Ca, Cb, Cc]

    Each region has:
        constraint 0: linearized reaction
    ...
    for _, g in df.groupby("region_id", sort=True):
        A = torch.tensor(
            g[["A_T", "A_Cao"]].to_numpy(dtype=float),
            dtype=torch.float32
        )
...
def get_violation(args, data, X, pred):
    """
    Return true/original constraint violations in original units.

    Output shape: (batch_size, 2)

    Column 0: original nonlinear reaction constraint residual
    Column 1: original mass-balance constraint residual

    Important:
    This does NOT compute the piecewise-linear residual.
    """

    return compute_violation_original_nonlinear(
        X_scaled=X,
        Ypred_scaled=pred,
        scaler=data["scaler"],
        device=X.device,
...
#     assert masks.shape[1] == len(A_list), (
#         f"Mask count {masks.shape[1]} != number of regions {len(A_list)}"
#     )
...
#         # add only this region's weighted violation
...
def compute_violation_original_nonlinear(
    ...
    """
    Returns original-unit residuals for:
        1) nonlinear reaction balance
