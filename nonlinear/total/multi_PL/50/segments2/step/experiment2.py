...
files_to_copy = ["main.py", "train.py", "models.py", "utils.py", "linearization.py", "data.csv", "ABb_matrices.csv", "region_edges.npz"]
...
def make_archive_dir(SCENARIO_ID: str, model_name: str, run_idx: int) -> str:
    ts = pd.Timestamp.utcnow().strftime("%Y%m%d-%H%M%S")
    leaf = f"{ts}_run{run_idx:02d}_{model_name}"
    path = os.path.join(ARCHIVE_ROOT, str(SCENARIO_ID), leaf)
...
def update_results_by_segments_csv(model_name: str, scenario_id: str,
                                   rmse_mean: float, viol_mean: float, viol_nl_mean: float,
                                   train_time_mean: float, exp_time_mean: float):
    col_rmse = f"{scenario_id}_RMSE_TOTAL"
    col_viol = f"{scenario_id}_VIOL"
    col_viol_nl = f"{scenario_id}_VIOL_NL"
    col_train_time = f"{scenario_id}_TRAIN_TIME_SEC"
    col_exp_time = f"{scenario_id}_EXP_TIME_SEC"
...
def main():
    ...
    update_results_by_segments_csv(
        "NN", SCENARIO_ID,
        nn_rmse_mean, nn_viol_mean, nn_viol_nl_mean,
        nn_train_time_mean, nn_exp_time_mean
)
    update_results_by_segments_csv(
        "KKThPINN", SCENARIO_ID,
        kkt_rmse_mean, kkt_viol_mean, kkt_viol_nl_mean,
        kkt_train_time_mean, kkt_exp_time_mean
)
