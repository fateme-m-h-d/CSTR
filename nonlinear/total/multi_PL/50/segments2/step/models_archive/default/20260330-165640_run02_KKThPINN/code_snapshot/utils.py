...
def LoadData(args):
    ...
    #           'shuffle': True}
    train_loader = data.DataLoader(dataset.train_set, **params_train)
    val_loader = data.DataLoader(dataset.val_set, **params_eval)
    test_loader = data.DataLoader(dataset.test_set, **params_eval)
...
def get_masks_2d(X, T_edges, C_edges, steepT=8e5, steepC=8e5, normalize=True, hard=False):
    T = X[:, 0:1]
    C = X[:, 1:2]
    ...
    for i in range(nT):
        if hard:
            wT = hard_window_1d(T, T_edges[i], T_edges[i+1], is_last=(i == nT - 1))
        else:
            wT = window_1d(T, T_edges[i], T_edges[i+1], steepT)

        for j in range(nC):
            if hard:
                wC = hard_window_1d(C, C_edges[j], C_edges[j+1], is_last=(j == nC - 1))
            else:
...
def get_violation(args, data, X, pred, steepT=8e5, steepC=8e5, hard=False):
    ...
    masks = get_masks_2d(X, T_edges, C_edges, steepT=steepT, steepC=steepC, normalize=True, hard=hard)
