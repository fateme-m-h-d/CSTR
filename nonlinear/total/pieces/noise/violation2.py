...
RANGES = [(280,300),(300,340),(340,360),(360,400),(400,460),(460,500), (500,530), (530,550), (550,565), (565,578), (578,590), (590,600)]
...
# calculation of nonlinear constraint
...
kf = Afo * np.exp(-Eaf/(R*TEMPS))
kr = Aro * np.exp(-Ear/(R*TEMPS))

# Ensure all arrays are float64 before calculation
Cao = np.float64(Cao)
Cbo = np.float64(Cbo)
Cco = np.float64(Cco)
kf = kf.astype(np.float64)
kr = kr.astype(np.float64)
Ca_raw = Ca_raw.astype(np.float64)
Cb_raw = Cb_raw.astype(np.float64)
Cc_raw = Cc_raw.astype(np.float64)
tau = np.float64(tau)

f1 = (Cao - Ca_raw
    - kf * Ca_raw * (Cb_raw**2) * tau
    + kr * (Cao - Ca_raw + Cbo - Cb_raw + Cco) * tau)

df = pd.read_csv(DATA_PATH)                        # shape (N,4)
raw_vals   = df.values                            # (N,4)
Ca = raw_vals[:, 1]  # Ca values
Cb = raw_vals[:, 2]  # Cb values
Cc = raw_vals[:, 3]  # Cc values
...
for i, T0 in enumerate(T_raw_check):
    # print(f"T = {T0:.1f} K")
    # print(f"  f(T,Ca,Cb) = {f1[i]:.6e}")
    # print("-"*55)
    plt.scatter(T0, f1[i], color='blue', label='f(T,Ca,Cb)' if i == 0 else "")
...
for i, T0 in enumerate(T_raw_check):
    ...
    # print(f"  f(T,Ca,Cb) = {f1[i]:.6e}")
    # print("-"*55)
    plt.scatter(T0, f2[i], color='red', label='f(T,Ca,Cb)' if i == 0 else "")
plt.title("Nonlinear Constraint Values vs Temperature - original data")
plt.show()
