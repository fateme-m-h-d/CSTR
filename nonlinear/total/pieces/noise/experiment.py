...
files_to_copy = ["main.py", "train.py", "models.py", "utils.py", "curves.py", "data.csv", "scaler.pkl", "moderate_noisy_data.csv", "load_data.py"]
...
def run_main(model_name, job, dataset_path_override=None):
    """
    ...
    dataset_path = dataset_path_override if dataset_path_override else "./data.csv"
    
    print(f"Running main.py with model={model_name} and job={job} (dataset_path={dataset_path}) ...")
    args = [
        "python", "main.py",
        "--model", model_name,
        "--model_id", "MODELID",
...
def run_model_experiments(model_name, num_iterations):
    ...
    for i in range(num_iterations):
        ...
        # Training run
        train_error = run_main(model_name, "train", dataset_path_override="./data.csv")
        if train_error is None:
            train_error = float('nan')
        training_errors.append(train_error)
        
        # Experiment run (with unseen data)
        exp_error = run_main(model_name, "experiment", dataset_path_override="./data.csv")
...
def get_predictions(model_name):
    ...
    scaler_path = os.path.join(target_folder, 'scaler.pkl')  # <-- use target_folder
    try:
