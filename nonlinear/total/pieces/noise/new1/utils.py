...
def load_data(dataset_path):
    ...
    if os.path.exists(scaler_path):
        # ---- NEW: reuse existing scaler (keeps train/test consistent) ----
        with open(scaler_path, 'rb') as f:
            scaler = pickle.load(f)
        print("Scaler loaded (no refit).")
    else:
        # ---- Fit ONCE (training) and save ----
        from sklearn.preprocessing import MaxAbsScaler
        scaler = MaxAbsScaler()
        scaler.fit(dataset)
        # keep your higher max for T (feature 0)
        scaler.scale_[0] = max(scaler.scale_[0], 800)
        with open(scaler_path, 'wb') as f:
            pickle.dump(scaler, f)
        print(f"Scaler fit & saved at {scaler_path}")

    print("Scaler Factors:", scaler.scale_)
    dataset = scaler.transform(dataset)

    # Save the transformed dataset to a CSV file (unchanged)
    scaled_dataset_path = os.path.join(os.getcwd(), 'scaled_data.csv')
    pd.DataFrame(dataset).to_csv(scaled_dataset_path, index=False)
...
def get_violation(args, data, X, pred, RANGES = [(280/800,300/800),(300/800,340/800),(340/800,360/800),(360/800,400/800),(400/800,460/800),(460/800,500/800),(500/800,530/800),(530/800,550/800),(550/800,565/800),(565/800,578/800),(578/800,590/800),(590/800,600/800)]):
