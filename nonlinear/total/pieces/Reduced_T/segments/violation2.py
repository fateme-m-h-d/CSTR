...
# def compute_piecewise_linear_violation(X_scaled_t, pred_scaled_t, A_list, B_list, b_list, T_edges_scaled):
#     x1d = X_scaled_t[:, 0].detach().cpu().numpy()
#     out = np.full(len(x1d), np.nan, dtype=np.float64)
...
#         v = (X_scaled_t @ A.T + pred_scaled_t @ B.T - b).detach().cpu().numpy().flatten()
#         out[mask] = np.abs(v[mask])

#     return out

def compute_piecewise_linear_violation(
    X_scaled_t, pred_scaled_t, A_list, B_list, b_list, T_edges_scaled, steepT=8e5
):
    ...
    v_all = []
    for i, (A, B, b) in enumerate(zip(A_list, B_list, b_list)):
        v = (X_scaled_t @ A.T + pred_scaled_t @ B.T - b)
        v_region = torch.abs(v * masks[:, i:i+1])
