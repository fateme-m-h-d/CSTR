    ...
    try:
        ...
        with torch.no_grad():
            for X, Y in data['test_loader']:
                ...
                v_nl = compute_violation_original_nonlinear(X, pred, data["scaler"], device=device) # shape (batch,)
...
def projection_sanity_check(data, args):
    """
    Sanity check only.

    No training.
    No saved model loading.
    No NN output.

    True Y enters the projection layer directly:
        pred = model(X, y_true=Y)

    Then we compare:
        |pred - Y|

    All metrics are averaged over all individual data points,
    not averaged over batch means.
    """
    ...
    with torch.no_grad():
        for X, Y in data["test_loader"]:
            ...
            # Ground-truth Y enters the projection layer.
            # NN is skipped inside forward().
            pred = model(X, y_true=Y)

            # ------------------------------------------------------------
            # Output-space projection error
            # ------------------------------------------------------------
            err = pred - Y

            abs_errors_all.append(torch.abs(err).detach().cpu())
            squared_errors_all.append((err ** 2).detach().cpu())

            # ------------------------------------------------------------
            # Normalized nonlinear violation from get_violation()
            # Shape: (batch, 1)
            # ------------------------------------------------------------
            pred_diff = get_violation(args, data, X, pred)
            violation_all.append(torch.abs(pred_diff).detach().cpu())

            # ------------------------------------------------------------
            # Original nonlinear violation
            # Shape: (batch,)
            # ------------------------------------------------------------
            v_nl = compute_violation_original_nonlinear(
                X, pred, data["scaler"], device=device
            )
            v_nl_all.append(v_nl.detach().cpu())

    # ------------------------------------------------------------
    # Concatenate all individual points
    # ------------------------------------------------------------
    abs_errors_all = torch.cat(abs_errors_all, dim=0)
    squared_errors_all = torch.cat(squared_errors_all, dim=0)

    violation_all = torch.cat(violation_all, dim=0).reshape(-1)
    v_nl_all = torch.cat(v_nl_all, dim=0).reshape(-1)

    # ------------------------------------------------------------
    # Average over all points, not over batches
    # ------------------------------------------------------------
    projection_mae_scaled = abs_errors_all.mean().item()
    projection_max_abs_scaled = abs_errors_all.max().item()
    projection_rmse_scaled = torch.sqrt(squared_errors_all.mean()).item()

    violation = violation_all.mean().item()
    violation_original_nonlinear = v_nl_all.mean().item()

    scores = {
        "projection_mae_scaled": float(projection_mae_scaled),
        "projection_max_abs_scaled": float(projection_max_abs_scaled),
        "projection_rmse_scaled": float(projection_rmse_scaled),
        "violation": float(violation),
        "violation_original_nonlinear": float(violation_original_nonlinear),
    }

    print("\n=== Projection sanity check ===")
    print("This uses true Y as input to the projection layer.")
    print("No NN prediction and no training are used.")
    print("All metrics are averaged over individual points, not batch means.\n")
