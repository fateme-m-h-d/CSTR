...
def LoadData(args):
    ...
    if args.job == "projection_check":
        # For projection sanity check, evaluate on the full dataset.
        # This makes it comparable with linearization_accuracy.py,
        # which reads all rows from data.csv.
...
def get_violation(args, data, X, pred, steepT=8e5, eps=1e-12):
    ...
    relative_residual = residual / 140.4526252746582
