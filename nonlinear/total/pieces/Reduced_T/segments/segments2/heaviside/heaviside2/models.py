...
class NNOPT(nn.Module):
    ...
    def forward(self, x, y_true=None, steepness=800000):
        """
        Normal use:
            model(x)
        ...
        If y_true is provided, the NN layers are skipped completely.
        The ground-truth output enters the projection layer directly.
        """
        ...
        if y_true is None:
            ...
            for layer in self.layers[:-1]:
                x0 = F.relu(layer(x0))
            y_input = self.layers[-1](x0)
        else:
            # Projection-only case: use ground-truth Y directly
            y_input = y_true
        ...
        fixed_outputs = []
        for i, (fc1, fc2) in enumerate(zip(self.fc1_list, self.fc2_list)):
            y_fixed = fc1(y_input) + fc2(x)
            mask = masks[:, i:i+1]
            fixed_outputs.append(y_fixed * mask)
