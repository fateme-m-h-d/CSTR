...
def safe_predict(model, X, steepness=800000):
    with torch.no_grad():
        return model(X, steepness=steepness) if isinstance(model, NNOPT) else model(X)
...
# ):
#     x1d = X_scaled_t[:, 0].detach().cpu().numpy()
#     N = len(x1d)
...
#         v_full = (X_scaled_t @ A.T + pred_scaled_t @ B.T - b).detach().cpu().numpy().flatten()

#         v_region = np.full(N, np.nan, dtype=np.float64)
#         v_region[mask] = np.abs(v_full[mask])
#         v_all.append(v_region)

#     return v_all

def compute_piecewise_linear_violation(
    X_scaled_t, pred_scaled_t, A_list, B_list, b_list, T_edges_scaled, steepT=8e5, plot_mask_eps=1e-20
):
    T_edges_scaled = torch.as_tensor(T_edges_scaled, dtype=X_scaled_t.dtype, device=X_scaled_t.device)
    masks = get_masks_1d(X_scaled_t, T_edges_scaled, steepT=steepT)
    ...
    for i, (A, B, b) in enumerate(zip(A_list, B_list, b_list)):
        ...
        v_region = (v * m).detach().cpu().numpy()
        m_np = m.detach().cpu().numpy()

        # display only where this region is actually active enough
        v_region[m_np < plot_mask_eps] = np.nan
        v_all.append(v_region)
    return v_all
...
def compute_violation_curve(model_name, archive_dir):
    ...
    if VIOLATION_KIND == "original_nonlinear":
        ...
        violation = compute_piecewise_linear_violation(
            X_scaled_t, pred_scaled_t, A_scaled, B_scaled, b_scaled, T_edges_scaled
        )
    else:
...
def plot_for_model(model_name, out_csv_prefix, out_plot_prefix):
    ...
    for nseg in SEGMENT_SCENARIOS:
        ...
        # ----------------------------------------
        # original nonlinear: one curve
        # ----------------------------------------
        if VIOLATION_KIND == "original_nonlinear":
            out_df = pd.DataFrame({
                "Temperature (T)": T_GRID,
                f"viol_nseg_{nseg}": violation
            })

            csv_path = f"{out_csv_prefix}_nseg_{nseg}.csv"
            plot_path = f"{out_plot_prefix}_nseg_{nseg}.png"

            out_df.to_csv(csv_path, index=False)

            plt.figure(figsize=(9, 5))
            plt.scatter(T_GRID, violation, linewidth=2, label=f"{nseg} segments")
            ...
            plt.title(f"{model_name}: original nonlinear violation vs temperature ({nseg} segments)")
            if USE_LOG_Y:
                plt.yscale("log")
        ...
        # ----------------------------------------
        # piecewise linear: one array per region
        # ----------------------------------------
        else:
            out_df = pd.DataFrame({"Temperature (T)": T_GRID})

            plt.figure(figsize=(9, 5))
            colors = plt.cm.Set3(np.linspace(0, 1, nseg))

            for i, v_region in enumerate(violation):
                out_df[f"v_region_{i+1}"] = v_region

                # line version
                plt.scatter(T_GRID, v_region, linewidth=2, color=colors[i], label=f"Region {i+1}")

                # if you want the old style more closely, use this instead:
                # plt.scatter(T_GRID, v_region, s=18, color=colors[i], label=f"Region {i+1}")
            ...
            all_vals = np.concatenate([np.nan_to_num(v, nan=np.nan) for v in violation])
            print(
                f"{model_name:10s} | nseg={nseg:2d} | "
                f"mean={np.nanmean(all_vals):.6e} | max={np.nanmax(all_vals):.6e}"
            )

            plt.xlabel("Input temperature (K)")
            plt.ylabel(r"Piecewise linear violation $|A x + B y - b|$")
            plt.title(f"{model_name}: piecewise linear violation vs temperature ({nseg} segments)")
            if USE_LOG_Y:
                plt.yscale("log")
            plt.grid(True, alpha=0.3)
            plt.legend()
