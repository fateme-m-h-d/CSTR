...
def get_masks_1d(X, T_edges, steepT=8e5, hard=False):
    """
    Returns masks of shape (batch, n_regions), consistent with current models.py.
    No normalization here, because your current models.py also does not normalize.
    """
    nT = len(T_edges) - 1
...
def get_violation(args, data, X, pred, steepT=8e5, hard=False):
    A_list, B_list, b_list = data['A_list'], data['B_list'], data['b_list']
    T_edges = torch.as_tensor(data['T_edges'], dtype=X.dtype, device=X.device)

    masks = get_masks_1d(X, T_edges, steepT=steepT, hard=hard)

    assert masks.shape[1] == len(A_list), \
        f"Mask count {masks.shape[1]} != #constraints {len(A_list)}"
