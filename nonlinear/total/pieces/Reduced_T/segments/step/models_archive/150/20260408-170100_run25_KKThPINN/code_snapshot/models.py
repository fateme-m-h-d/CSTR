...
class NNOPT(nn.Module):
    ...
    def get_masks(self, x, steepness=800000, hard=False):
        ...
        for i in range(n):
            if hard:
                mask = self.hard_window_1d(x, self.T_edges[i], self.T_edges[i+1], is_last=(i == n - 1))
    ...
    def forward(self, x, steepness=800000, hard=False):
        x0 = x                                             # input               
