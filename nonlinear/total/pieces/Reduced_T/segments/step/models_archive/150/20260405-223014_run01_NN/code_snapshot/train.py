...
def run_training(args, data):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (X, Y) in enumerate(data['train_loader']):
            ...
            train_violation += torch.abs(pred_diff).sum(dim=1).mean()
        ...
        with torch.no_grad():
            for X, Y in data['test_loader']:
                ...
                if args.model == 'KKThPINN':
                    ...
                    pred_diff = get_violation(args, data, X, pred, hard=False)  # ← CHANGED: pass hard=False to get_violation for KKThPINN
                else:
                    pred = model(X)
