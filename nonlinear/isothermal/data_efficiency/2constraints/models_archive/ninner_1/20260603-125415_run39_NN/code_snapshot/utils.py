...
def compute_violation_original_nonlinear(X_scaled, Ypred_scaled, scaler, device="cpu"):
    """
    Returns |f_nl| per sample in original units for the isothermal reaction equation.

    Uses the predicted Cc directly:
        f = Cao - Ca - kf*Ca*Cb^2*tau + kr*Cc*tau

    Mass balance is already measured through the second PL constraint.
    """
    with torch.no_grad():
