...
def load_ABb_from_csv(csv_path="ABb_matrices.csv"):
    ...
    if "constraint_order" in df.columns:
        df = df.sort_values(["region_id", "constraint_order"]).reset_index(drop=True)
    else:
        df = df.sort_values("region_id").reset_index(drop=True)

    A_list, B_list, b_list = [], [], []

    for region_id, g in df.groupby("region_id", sort=True):
        A = torch.tensor(
            g[["A_Cao"]].to_numpy(),
            dtype=torch.float32
        )
        ...
        B = torch.tensor(
            g[["B_Ca", "B_Cb", "B_Cc"]].to_numpy(),
            dtype=torch.float32
        )

        b = torch.tensor(
            g["b"].to_numpy().reshape(-1, 1),
            dtype=torch.float32
        )
...
def get_violation(args, data, X, pred):
    ...
    n_constraints = B_list[0].shape[0]
    ...
    for r, (Ai, Bi, bi) in enumerate(zip(A_list, B_list, b_list)):
        ...
        # v shape: (batch_size, n_constraints)
        v = X @ Ai.T + pred @ Bi.T - bi.view(1, -1)
