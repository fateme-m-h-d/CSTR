...
from utils import LoadData, LoadModel, get_violation, compute_violation_original_nonlinear
...
def projection_only_forward(model, X, Y):
    """
    Projection-only forward pass.

    This skips the neural network and uses:
        y_input = y_true

    Then applies the fixed projection branches:
        y_proj = Astar*x + Bstar*y_true + bstar
    with the active Heaviside region mask.
    """

    # Use the same hard masks stored inside the model
...
def run_projection_check(args, data):
    """
    Projection-only diagnostic.

    Computes:
    1. scaled output MAE: |y_proj - y_true|
    2. PL constraint violation using get_violation()
    3. original nonlinear violation using compute_violation_original_nonlinear()

    The averaging is done over all individual data points, not over batches.
    """
    ...
    with torch.no_grad():
        for X, Y in data["test_loader"]:
            ...
            # projection-only: y_input = y_true
