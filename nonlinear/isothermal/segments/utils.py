...
def replace_Cc_with_mass_balance(X_scaled, Ypred_scaled, scaler, device="cpu"):
    """
    Replace predicted Cc using total mass balance:

        Cc = Cao - Ca + Cbo - Cb + Cco

    The calculation is done in original units, then transformed back
    to scaled units so the RMSE stays comparable with the current RMSE.
    """
    with torch.no_grad():
        torch_dtype = Ypred_scaled.dtype

        XYs_pred = torch.cat([X_scaled, Ypred_scaled], dim=1).detach().cpu().numpy()
        XY_pred = scaler.inverse_transform(XYs_pred)

        Cao = XY_pred[:, 0]
        Ca_pred = XY_pred[:, 1]
        Cb_pred = XY_pred[:, 2]

        Cc_pred_mb = Cao - Ca_pred + Cbo - Cb_pred + Cco
        XY_pred[:, 3] = Cc_pred_mb

        XYs_pred_mb = scaler.transform(XY_pred)
...
def load_data(dataset_path):
    ...
    pd.DataFrame(dataset).to_csv(os.path.join(os.getcwd(), "scaled_data.csv"), index=False)
...
def load_ABb_from_csv(csv_path="ABb_matrices.csv"):
    ...
    for _, row in df.iterrows():
        A_list.append(torch.tensor([[row["A_Cao"]]], dtype=torch.float32))
...
def get_violation(args, data, X, pred):
    ...
    active_violation = torch.zeros((X.shape[0], 1), dtype=X.dtype, device=X.device)
...
def compute_violation_original_nonlinear(X_scaled, Ypred_scaled, scaler, device="cpu"):
    """Returns |f_nl| per sample in original units for the isothermal case."""
    with torch.no_grad():
        ...
        return torch.tensor(np.abs(eq1), dtype=torch_dtype, device=device)
