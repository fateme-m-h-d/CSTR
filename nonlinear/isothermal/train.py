...
from utils import LoadModel, get_optimizer, get_loss_func, get_violation, PINNLoss, ALMLoss, compute_violation_original_nonlinear,   replace_Cc_with_mass_balance
...
def optimizer_step(model, optimizer, loss_func, X, Y, args, data, lambda_k=None, mu_k=None):
    if isinstance(loss_func, PINNLoss):
        ...
        (mse_loss + pinn_loss).backward(); optimizer.step()
...
def test(model, data, args):
    ...
    with torch.no_grad():
        for X, Y in data["val_loader"]:
            ...
            test_violation += torch.abs(pred_diff).mean()

    return test_loss / len(data["val_loader"]), test_violation / len(data["val_loader"])
    ...
    try:
        ...
        with torch.no_grad():
            for X, Y in data["test_loader"]:
                X, Y = X.to(device), Y.to(device)
                pred = model(X)
                pred_mb_Cc = replace_Cc_with_mass_balance(
                    X, pred, data["scaler"], device=device
                )

                pred_diff = get_violation(args, data, X, pred)
                rmse_total += loss_func(pred, Y).item()
                rmse_total_mb_Cc += loss_func(pred_mb_Cc, Y).item()
                violation += torch.abs(pred_diff).mean()
                v_nl = compute_violation_original_nonlinear(X, pred, data["scaler"], device=device)
                v_nl_batches.append(v_nl.mean().item())

        rmse_total = np.sqrt(rmse_total / len(data["test_loader"]))
        rmse_total_mb_Cc = np.sqrt(rmse_total_mb_Cc / len(data["test_loader"]))
        ...
        scores.update({
            "rmse_total": float(rmse_total),
             "rmse_total_mb_Cc": float(rmse_total_mb_Cc),
