...
def main():
    ...
    for n_t_regions in SEGMENT_SCENARIOS:
        # Keep the same total region counts as the previous nT x N_C_REGIONS
        # experiments, but let the new 2D partitioner place them arbitrarily.
        n_regions = n_t_regions * N_C_REGIONS
        print(f"Running adaptive full-cell partition with n_regions={n_regions}")
        shutil.copy2(BASE_DIR / "data_fixed.csv", BASE_DIR / "data.csv")
        run([
            sys.executable,
            "-m", "src.linearization",
            "--n_regions", n_regions,
            "--partition", "taylor_rectangles",
        ])

        env = os.environ.copy()
        env["SCENARIO_ID"] = f"nreg_{n_regions}"
        ...
        row = {
            "legacy_nT_equivalent": n_t_regions,
