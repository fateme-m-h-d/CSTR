...
from models import NN, NNOPT, get_rectangle_masks
...
def load_region_bounds(npz_path="region_edges.npz"):
    ...
    if "region_bounds" not in artifact.files:
        raise ValueError(
            f"{npz_path} does not contain 'region_bounds'. "
            "Run the new rectangle-wise src.linearization first."
        )
...
def LoadData(args):
    ...
    region_bounds_raw = load_region_bounds()
    region_bounds = region_bounds_raw.copy()
    region_bounds[:, 0:2] /= scaler.scale_[0]  # T low/high
    region_bounds[:, 2:4] /= scaler.scale_[1]  # Cao low/high
    ...
    assert n_regions == len(A_list), (
        ...
        "Fix region_edges.npz or ABb_matrices.csv."
    )
...
def get_violation(args, data, X, pred):
    masks = get_rectangle_masks(X, data["region_bounds"])
