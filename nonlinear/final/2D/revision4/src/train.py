...
def validation_original_nonlinear(
    model,
    data,
    constraint_index=0,
):
    """
    Compute the original nonlinear constraint violation
    on the validation set using the FINAL in-memory model.

    Parameters
    ----------
    model:
        Trained model returned by run_training().

    data:
        Dictionary returned by LoadData().

    constraint_index:
        0 -> reaction constraint only
        1 -> mass-balance constraint only
        None -> mean over all original constraints

    Returns
    -------
    float
        Mean absolute original nonlinear constraint violation
    ...
    with torch.no_grad():
        ...
        for X, _ in data["val_loader"]:
            ...
            if constraint_index is None:
                selected = nonlinear_violation.reshape(-1)
            else:
                selected = nonlinear_violation[
                    :,
                    constraint_index,
                ]

            violation_sum += selected.sum().item()
            violation_count += selected.numel()

    if violation_count == 0:
        raise RuntimeError(
            "Validation set contains no samples."
        )

    return violation_sum / violation_count
