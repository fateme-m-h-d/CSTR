...
def optimizer_step(model, optimizer, loss_func, X, Y, args, data, lambda_k=None, mu_k=None):
    if isinstance(loss_func, PINNLoss):
        ...
        #new new
        # # -------- NEW WEIGHTED-MSE LOGIC --------
        # if args.model == 'NN':
        #     # NN: same as before, only first 3 targets with standard MSE
        #     Y_eff = Y[:, :args.z0_inner_dim]
        #     loss = loss_func(pred, Y_eff)

        # else:
        #     # KKThPINN: weighted loss across (Ca,Cb,Cc) vs (f,g)
        #     # pred, Y are [N, 5] in scaled space

        #     Y_eff = Y  # all 5 targets

        #     # element-wise squared error
        #     criterion = nn.MSELoss(reduction='none')
        #     err = criterion(pred, Y_eff)      # shape [N, 5]

        #     # ---- choose your weights here ----
        #     # for example:
        #     w3  = 1.0   # weight for Ca, Cb, Cc
        #     wfg = 0   # weight for f, g   (example; change as you like)
        ...
        #     # broadcast w over batch, then average
        #     loss = (err * w).mean()
        # # -------- END NEW LOGIC --------
        loss.backward()
        optimizer.step()
