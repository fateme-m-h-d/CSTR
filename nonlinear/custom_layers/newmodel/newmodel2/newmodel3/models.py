...
"""
new model because I changed the model stcructre based on what hector asked for  adding weights to the model. returning both 3 and 5 outputs.
"""
...
class NNOPT(nn.Module):
    def __init__(self, input_dim, hidden_dim, hidden_num, z0_inner_dim, z0_dim, A, B, b):
        ...
        self.eps9999 = torch.tensor(float("0." + "9" * 15))
        ...
        self.Bstar = -  torch.mm(self.chunk, self.B)   # -W* 𝐵𝑇 (𝐵𝐵𝑇 )-1 * B
        self.bstar = torch.matmul(self.chunk, self.b).squeeze(-1)                  # w* 𝐵𝑇 (𝐵𝐵𝑇 )-1 * b    # I don't understand .squeeze(-1)   
