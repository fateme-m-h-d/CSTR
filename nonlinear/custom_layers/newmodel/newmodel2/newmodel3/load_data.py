...
def make_prediction(model, scaler, temperature):
    ...
    returns (on original / inverse-scaled units):
      - NN:      array (N, 3)  = [Ca, Cb, Cc]
      - KKThPINN: array (N, 5) = [Ca, Cb, Cc, f, g]
    """
    # --- 1) normalize temperature(s) exactly like during training ---
    ...
    # Build a dummy [T, Ca, Cb, Cc, f, g] so we can reuse the same scaler
    dummy_full = np.zeros((n, 6), dtype=float)
    dummy_full[:, 0] = temperature[:, 0]

    # Scale using the fitted MaxAbsScaler
    transformed = scaler.transform(dummy_full)
    T_norm = transformed[:, :1]  # first feature is T, scaled

    # --- 2) run the model in scaled space ---
    X = torch.tensor(T_norm, dtype=torch.float64).to(device)
    with torch.no_grad():
        out = model(X)

    # Handle NN vs KKThPINN:
    #   - NN:   out is a single tensor [N,3]
    #   - KKT:  out is a tuple (z3, z5); z5 has [Ca,Cb,Cc,f,g]
    if isinstance(out, tuple):
        # KKThPINN: (z3, z5)
        _, z5 = out
        out_arr = z5.cpu().numpy()   # shape (N, 5)
    else:
        # Plain NN: only Ca, Cb, Cc
        out_arr = out.cpu().numpy()  # shape (N, 3)

    num_out = out_arr.shape[1]
    if num_out not in (3, 5):
        raise ValueError(f"Unexpected model output width {num_out} (expected 3 or 5).")

    # Put predictions back into dummy array so inverse_transform works
    dummy_full[:, 1:1 + num_out] = out_arr

    # --- 3) inverse-scale to physical units and return only outputs ---
