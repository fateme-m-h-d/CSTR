...
#     new_temperatures = np.linspace(280, 600, 100)  # 300 points
...
def get_predictions(model_name):
    """
    Use the same scaler + model interface as training/evaluation:

      - Loads scaler.pkl
      - Loads the trained model from ./new1/models/...
      - Uses load_data.make_prediction(...) to get predictions in ORIGINAL units

    Returns:
      T_test : (N,1) temperatures in K
      Y_pred : (N,3) concentrations [Ca, Cb, Cc] in mol/L
               (for KKT we discard f,g and keep first 3 columns)
    """
    # 1) load the scaler used during training
    scaler_path = os.path.join(os.getcwd(), 'scaler.pkl')
    with open(scaler_path, 'rb') as f:
        scaler = pickle.load(f)

    # 2) set model hyperparameters (must match training)
    input_dim = 1
    hidden_dim = 32
    hidden_num = 2
    z0_dim = 5
    ...
    # 3) choose model path + type + (A,B,b) for KKT
    if model_name == "NN":
        model_path = "./new1/models/cstr/NN/0.2/MODELID_0.2_0.pth"
        model_type = "NN"
        A = B = b = None
        ...
        # A,B,b just need correct shapes; values are ignored once
        # checkpoint weights are loaded (fc_fixed layers are frozen).
        A = torch.tensor([[0.0]], dtype=torch.float64)
        B = torch.tensor([[1.0, 0.0, 0.0, -10.0, 10.0]], dtype=torch.float64)
        b = torch.tensor([1.0], dtype=torch.float64)
    else:
        raise ValueError(f"Unknown model_name: {model_name}")

    # 4) load model with saved weights (same as in training)
    model = load_saved_model(
        model_path, model_type,
        input_dim, hidden_dim, hidden_num,
        z0_inner_dim, z0_dim, A, B, b
    )
    model = model.to(torch.float64)
    model.eval()

    # 5) build temperature grid and call make_prediction
    T_test = np.linspace(280, 600, 50)  # K
    preds_full = make_prediction(model, scaler, T_test)  # (N,3) or (N,5) in ORIGINAL units

    # Keep only Ca,Cb,Cc for plotting
    Y_pred = preds_full[:, :3]

    return T_test.reshape(-1, 1), Y_pred


# === Main Execution ===

def main():
    num_iterations = 100  # Perform 50 cycles for each model
