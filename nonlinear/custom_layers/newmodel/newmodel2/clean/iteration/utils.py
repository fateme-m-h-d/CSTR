...
def LoadModel(args, data):
    if args.model == 'NN':
        ...
        # Optional: unroll a few "project → recompute f,g" steps inside NNOPT.
        proj_iters = getattr(args, 'proj_iters', 0)
        debug_delta = getattr(args, 'debug_delta', False)
        model = NNOPT(
            args.input_dim, args.hidden_dim, args.hidden_num,
    ...
    model.proj_iters = getattr(args, "proj_iters", 0)
    model.debug_delta = getattr(args, "debug_delta", False)
...
# IMPORTANT:
# Do NOT call load_data() at import time. The training pipeline should create and
# carry a single scaler via LoadData(...) -> data['scaler'].
# Import-time side effects can silently create a second scaler and break consistency.
...
class Data_cstr(data.Dataset):
    def __init__(self, dataset):
        ...
        self.B = torch.tensor([[ 1, 0, 0, -10, +10]
                                ])  #changed
        self.b = torch.tensor([1])    #changed
