...
class NNOPT(nn.Module):
    def __init__(self, input_dim, hidden_dim, hidden_num, z0_inner_dim, z0_dim, A, B, b, scaler):
        ...
        # chunk = M B^T (B M B^T)^{-1}
        # -----------------------------

        # choose weights (example: discourage changing f,g)
        # w shape: [z0_dim] and must be positive
        w = torch.tensor([1e6, 1.0, 1.0, 1e-8, 1e-8], device=device, dtype=self.B.dtype)
       
        # M = W^{-1}  (diagonal)
        M = torch.diag(1.0 / w)   # [z0_dim, z0_dim]

        # S = B M B^T  (size [m,m])
        S = torch.mm(self.B, torch.mm(M, self.B.t()))

        # chunk = M B^T (B M B^T)^{-1}   (size [z0_dim, m])
        self.chunk = torch.mm(torch.mm(M, self.B.t()), torch.inverse(S))
