...
def solve_cb_from_BDH(B: torch.Tensor, D: torch.Tensor, H: torch.Tensor,
                      kf: torch.Tensor, kr: torch.Tensor):
    """
    Forward solver for implicit layer:
      1) compute all analytic roots
      2) pick the best physical root using your CSTR residual criterion
    """
    cb_cands = cb_roots_from_BDH(B, D, H)
    cb_best, ok = pick_cb_by_cstr_residual(cb_cands, D, kf, kr)
    return cb_best, ok
...
def solve_cb_ca_g_from_fh(T_scaled: torch.Tensor, z5: torch.Tensor, scaler):
    ...
    Cb_un = CbFromBDHImplicit.apply(B, D, H, kf, kr)


    # ok mask for logging only (not for gradients)
    ok = torch.isfinite(Cb_un)
...
class CbFromBDHImplicit(torch.autograd.Function):
    @staticmethod
    def forward(ctx, B, D, H, kf, kr):
        # SAME forward solver you already validated (analytic + root selection)
        cb, ok = solve_cb_from_BDH(B, D, H, kf, kr)
        ctx.save_for_backward(B, D, H, cb)
        return cb

    @staticmethod
    def backward(ctx, grad_cb):
        B, D, H, cb = ctx.saved_tensors

        # dP/dcb
        dP_dcb = 3.0 * B * cb**2 - 2.0 * B * D * cb + 1.0

        # sign-preserving floor (DO NOT use clamp_min here)
        eps = 1e-12
        sgn = torch.sign(dP_dcb)
        sgn = torch.where(sgn == 0, torch.ones_like(sgn), sgn)
        denom = torch.where(dP_dcb.abs() < eps, eps * sgn, dP_dcb)

        # partials
        dP_dB = cb**3 - D * cb**2
        dP_dD = -B * cb**2 - 1.0
        dP_dH = torch.ones_like(cb)
        ...
        grad_B = grad_cb * dcb_dB
        grad_D = grad_cb * dcb_dD
        grad_H = grad_cb * dcb_dH

        # no grads for kf, kr (they are used only for root selection)
        return grad_B, grad_D, grad_H, None, None
