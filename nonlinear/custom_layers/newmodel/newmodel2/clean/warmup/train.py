...
def append_pred_row_csv(big_path: str, phase: str, epoch: int, batch_idx: int,
                        y_true_t: torch.Tensor, y_pred_t: torch.Tensor, predhat_t: torch.Tensor):
    """
    ...
    predhat = predhat_t.detach().cpu().numpy()      # [N, D]
    N, D = y_true.shape
    ...
    with open(big_path, "a", newline="") as f:
        ...
        if write_header:
            header = (["phase","epoch","batch_idx","row","row_MSE"]
                      ...
                      + [f"predhat_{j}" for j in range(D)]
                      )
            w.writerow(header)
        for r in range(N):
            w.writerow([phase, epoch, batch_idx, r, float(row_mse[r]),
                        *y_true[r].tolist(), *y_pred[r].tolist(), *predhat[r].tolist()])
...
def forward_kkt_style(model, X, args, data):
    ...
    """
    if isinstance(model, NNOPT):
        pred3, pred5, predhat = model(X)
        return pred3, pred5, predhat
    else:
        pred3 = model(X)
        predhat = pred3
        pred5 = _augment_with_fg_from_nn(X, pred3, data, args)
        return pred3, pred5, predhat

@torch.no_grad()
def copy_nn_to_nnopt(model_nn: NN, model_nnopt: NNOPT):
    """
    Copies the backbone NN weights (model_nn.layers) into model_nnopt.layers.
    Assumes same architecture: same number of layers and matching shapes.
    """
    if len(model_nn.layers) != len(model_nnopt.layers):
        raise ValueError("NN and NNOPT do not have the same number of backbone layers.")

    for l1, l2 in zip(model_nn.layers, model_nnopt.layers):
...
def run_training_staged(args, data):

    """
    Stage 1: train NN for args.warmup_epochs
    Stage 2: copy NN -> NNOPT, then train NNOPT for the remaining epochs
    Optional: smooth blend during args.blend_epochs
    """
    ...
    # --- Make models explicitly (Option A) ---
    model1 = NN(args.input_dim, args.hidden_dim, args.hidden_num, args.z0_inner_dim)
    model2 = NNOPT(args.input_dim, args.hidden_dim, args.hidden_num,
                   args.z0_inner_dim, args.z0_dim,
                   data['A'], data['B'], data['b'], data['scaler'])
    model1 = model1.to(device).double()
    model2 = model2.to(device).double()

    opt1 = get_optimizer(args, model1)
    opt2 = get_optimizer(args, model2)
    ...
    blend = int(args.blend_epochs) if args.switch_mode == 'smooth' else 0
    ...
    for epoch in range(args.epochs):
        ...
        # decide stage and alpha
        in_stage1 = epoch < warmup
        in_blend = (not in_stage1) and (blend > 0) and (epoch < warmup + blend)

        if (not in_stage1) and (not copied):
            copy_nn_to_nnopt(model1, model2)
            copied = True
            print(f"Copied NN -> NNOPT at epoch {epoch+1}")

        # alpha schedule: 1 -> 0 over blend epochs
        if in_blend:
            # epoch=warmup => alpha=1, epoch=warmup+blend-1 => alpha=0
            denom = max(blend - 1, 1)
            progress = (epoch - warmup) / denom
            alpha = float(1.0 - progress)
        else:
            alpha = 0.0  # hard switch or after blending => pure model2
        ...
        for batch_idx, (X, Y) in enumerate(data['train_loader']):
            ...
            if in_stage1:
                ...
                pred3_1, _, _ = forward_kkt_style(model1, X, args, data)
                loss = loss_func(pred3_1, Y_eff)
                loss.backward()
                opt1.step()
                
                pred_for_violation = pred3_1

            else:
                model2.train()
                opt2.zero_grad()

                pred3_2, _, _ = forward_kkt_style(model2, X, args, data)
                
                if in_blend:
                    model1.eval()
                    with torch.no_grad():
                        pred3_1, _, _ = forward_kkt_style(model1, X, args, data)
                    pred = alpha * pred3_1 + (1.0 - alpha) * pred3_2
                else:
                    pred = pred3_2
            ...
            train_violation += float(torch.abs(pred_diff.view(-1)).mean().item())
        ...
        # ---- VALIDATION ----
        val_loss, val_violation = test_staged(model1, model2, data, args, epoch, warmup, blend)
        epoch_val_rmse = float(np.sqrt(val_loss))
        
        train_losses.append(float(train_loss)), train_violations.append(float(train_violation))
        val_losses.append(float(val_loss)), val_violations.append(float(val_violation))

        if epoch >= warmup:
            checkpoint(model2, val_loss, min_loss, args, epoch)


        if (epoch + 1) % 50 == 0:
            stage_str = "stage1(NN)" if in_stage1 else ("blend" if in_blend else "stage2(NNOPT)")
...
def run_training(args, data):
    ...
    if args.model == "KKThPINN" and getattr(args, "warmup_epochs", 0) > 0:
        return run_training_staged(args, data)
    # otherwise do the normal single-model training
...
def test_staged(model1, model2, data, args, epoch, warmup, blend):
    loss_func = get_loss_func(args, data)
    model1.eval()
    model2.eval()
    ...
    #             print("DEBUG dataset f,g mismatch (should be ~0) mean:", diff_mean, "max:", diff_max)
            ...
            pred_diff = get_violation(args, data, X, pred)
            test_violation += float(torch.abs(pred_diff.view(-1)).mean().item())
