...
"""
new model because I changed the model stcructre based on what hector asked for  adding weights to the model. But still returning 5 outputs.
"""
...
class NNOPT(nn.Module):
    def __init__(self, input_dim, hidden_dim, hidden_num, z0_inner_dim, z0_dim, A, B, b):
        ...
        self.eps9999 = torch.tensor(float("0." + "9" * 12))
        ...
        self.Bstar = - self.eps9999 * torch.mm(self.chunk, self.B)   # -W* 𝐵𝑇 (𝐵𝐵𝑇 )-1 * B
        self.bstar = self.eps9999 * torch.matmul(self.chunk, self.b).squeeze(-1)                  # w* 𝐵𝑇 (𝐵𝐵𝑇 )-1 * b    # I don't understand .squeeze(-1)   
        ...
        # self.fc_fixed3 = nn.Linear(5, z0_dim, bias=False)  ########    # input dimensions
    ...
    def forward(self, x):
        ...
        z_full = self.fc_fixed1(basis_outputs) + self.fc_fixed2(x) + basis_outputs
        z = z_full[:, :3]   # keep first 3 components
        # z: [batch, 3]
