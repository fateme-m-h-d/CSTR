...
from train_weighted import run_training, evaluate_model
import argparse
import time
import copy
...
def add_arguments():
    ...
    # --- Weighted MSE (per-output weights; useful to emphasize f and g) ---
    parser.add_argument('--use_weighted_mse', action='store_true',
                        help='If set, replace plain MSELoss with a per-output weighted MSE (in scaled space).')
    parser.add_argument('--nn_loss_on_aug5', action='store_true',
                        help='(NN only) If set, compute f,g from (Ca,Cb,Cc) and train on 5D [Ca,Cb,Cc,f,g].')
    parser.add_argument('--w_ca', type=float, default=1.0, help='Weight for Ca in MSE.')
    parser.add_argument('--w_cb', type=float, default=1.0, help='Weight for Cb in MSE.')
    parser.add_argument('--w_cc', type=float, default=1.0, help='Weight for Cc in MSE.')
    parser.add_argument('--w_f',  type=float, default=1.0, help='Weight for f in MSE.')
    parser.add_argument('--w_g',  type=float, default=1.0, help='Weight for g in MSE.')
