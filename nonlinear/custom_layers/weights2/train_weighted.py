...
def _build_mse_weights(args, D: int, device, dtype):
    """Return a length-D tensor of per-output weights in the order [Ca, Cb, Cc, f, g]."""
    # Defaults (all 1.0)
    w = torch.ones(D, device=device, dtype=dtype)
    if D >= 1: w[0] = float(getattr(args, "w_ca", 1.0))
    if D >= 2: w[1] = float(getattr(args, "w_cb", 1.0))
    if D >= 3: w[2] = float(getattr(args, "w_cc", 1.0))
    if D >= 4: w[3] = float(getattr(args, "w_f",  1.0))
    if D >= 5: w[4] = float(getattr(args, "w_g",  1.0))
    return w

def weighted_mse(pred: torch.Tensor, target: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
    """Weighted MSE that matches nn.MSELoss(reduction='mean') semantics, but per-output weighted.

    pred/target: [N, D]
    w:           [D] (broadcasted)
    Returns a scalar tensor.
    loss = mean_{i,j} [ w_j * (pred_ij - target_ij)^2 ]
    """
    se = (pred - target) ** 2                      # [N,D]
    return (se * w.view(1, -1)).mean()             # divide by N*D (NOT by sum(w))
...
def optimizer_step(model, optimizer, loss_func, X, Y, args, data, lambda_k=None, mu_k=None):
    if isinstance(loss_func, PINNLoss):
        ...
        # pred is [N,5] for KKThPINN
        # fg_pred = pred[:, 3:5]

        # # recompute fg_star from Ca,Cb,T (scaled fg, same as your model uses)
        # pred5_star = _augment_with_fg_from_nn(X, pred[:, :3], data, args)  # returns [Ca,Cb,Cc,f*,g*] scaled
        # fg_star = pred5_star[:, 3:5]
        # Yfg = Y[:, 3:5]

        # fg_incons = (fg_pred - fg_star).abs().mean().item()
        # fg_incons2 = (fg_pred - Yfg).abs().mean().item()
        # print("mean |fg_inconsistency| =", fg_incons)
        # print("mean |fg_inconsistency to labels| =", fg_incons2)


        # Default (old behavior): NN trains on only the first z0_inner_dim targets (Ca,Cb,Cc),
        # while KKThPINN trains on all 5 outputs.
        if getattr(args, "use_weighted_mse", False):
            # Choose what we compare against (still in *scaled* space)
            if args.model == 'NN' and getattr(args, "nn_loss_on_aug5", False):
                pred_use = _augment_with_fg_from_nn(X, pred, data, args)  # [N,5]
                target_use = Y                                            # [N,5]
            else:
                pred_use = pred
                target_use = Y[:, :args.z0_inner_dim] if args.model == 'NN' else Y

            w = _build_mse_weights(args, pred_use.shape[1], pred_use.device, pred_use.dtype)
            loss = weighted_mse(pred_use, target_use, w)
        else:
            target_use = Y[:, :args.z0_inner_dim] if args.model == 'NN' else Y
            loss = loss_func(pred, target_use)
...
def test(model, data, args, current_epoch=None):
    ...
    #             print("DEBUG dataset f,g mismatch (should be ~0) mean:", diff_mean, "max:", diff_max)
            ...
            if args.loss_type == 'PINN':
                ...
                if getattr(args, 'use_weighted_mse', False):
                    if args.model == 'NN' and getattr(args, 'nn_loss_on_aug5', False):
                        pred_use = _augment_with_fg_from_nn(X, pred, data, args)
                        target_use = Y
                    else:
                        pred_use = pred
                        target_use = Y_eff
...
def evaluate_model(data, args):
    ...
    violation = 0
    rmse_inner = 0.0
    rmse_fg = 0.0  # RMSE over [f,g] in scaled space
    # viol_max = 0.0    
    ...
    try:
        ...
        with torch.no_grad():
            for batch_idx, (X, Y) in enumerate(data['test_loader']):
                ...
                # f,g RMSE (scaled space) for monitoring
                if args.model == 'NN':
                    pred5_for_fg = _augment_with_fg_from_nn(X, pred, data, args)
                else:
                    pred5_for_fg = pred
                loss_fg = loss_func(pred5_for_fg[:, 3:5], Y[:, 3:5])
                rmse_fg += loss_fg.item()
    ...
    # Recompute f,g from the *labels* Ca,Cb,Cc using your existing function
        ...
        rmse_fg /= len(data['test_loader'])
        rmse_fg = np.sqrt(rmse_fg)
