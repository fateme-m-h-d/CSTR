...
def _augment_with_fg_from_nn(X, pred3, data, args):
    ...
    scale_h = torch.as_tensor(sc.scale_[4], device=pred3.device, dtype=pred3.dtype)

    # Build g and f
    g = (kf / scale_h) * Ca_unscaled * (Cb_unscaled ** 2)
    f = (kr / scale_h) * (Cao_t - Ca_unscaled + Cbo_t - Cb_unscaled + Cco_t)
    h = (f - g)

    return torch.cat([Ca, Cb, Cc, h], dim=1)


import math
import csv

def eval_dim_metrics(model, loader, device, tag):
    model.eval()
    D = 4  # [Ca, Cb, Cc, h]  (scaled space)
    mse_inner = torch.zeros(D, dtype=torch.float64)
    mse_post  = torch.zeros(D, dtype=torch.float64)
    n = 0
    max_pred = 0.0
    max_Y    = 0.0
    max_proj_delta = 0.0
    ...
    with torch.no_grad():
        for X, Y in loader:
            X = X.to(device)
            Y = Y.to(device)
            pred = model(X)  # triggers caching of inner3 & h
            inner = torch.cat([model._last_inner3, model._last_h], dim=1)

            # accumulate (sample-weighted)
            mse_inner += ((inner - Y)**2).sum(0).double().cpu()
            mse_post  += ((pred  - Y)**2).sum(0).double().cpu()
            n += Y.shape[0]

            # consistency check: pred == B*inner + A*X + b
            post_rebuild = model.fc_fixed1(inner) + model.fc_fixed2(X)
            max_proj_delta = max(max_proj_delta,
                                 float((pred - post_rebuild).abs().max().cpu()))

            max_pred = max(max_pred, float(pred.abs().max().cpu()))
            max_Y    = max(max_Y,    float(Y.abs().max().cpu()))

    rmse_inner = torch.sqrt(mse_inner / max(n,1)).tolist()
    rmse_post  = torch.sqrt(mse_post  / max(n,1)).tolist()

    print(f"[{tag}] RMSE dims (inner): {rmse_inner}")   # [Ca,Cb,Cc,h]
    print(f"[{tag}] RMSE dims (post ): {rmse_post}")    # [Ca,Cb,Cc,h]
    print(f"[{tag}] max|pred| vs max|Y|: {max_pred:.6g} vs {max_Y:.6g}")
    print(f"[{tag}] max|pred - (B*inner + A*X + b)|: {max_proj_delta:.3e}")

    return rmse_inner, rmse_post, max_pred, max_Y, max_proj_delta
...
def run_training(args, data):
    ...
    for epoch in range(args.epochs):
        ...
        # after each epoch, or at the very end
        # print("\n=== Final train/val diagnostics ===")
        # train_rmse_in, train_rmse_post, _, _, _ = eval_dim_metrics(model, data['train_loader'], device, "train")
        # val_rmse_in,   val_rmse_post,   _, _, _ = eval_dim_metrics(model, data['val_loader'],   device, "val")
...
def optimizer_step(model, optimizer, loss_func, X, Y, args, data, lambda_k=None, mu_k=None):
    if isinstance(loss_func, PINNLoss):
        ...
        # NEWNEW: split loss for KKThPINN
        # if args.model == 'NN':
        #     # NN stays exactly the same (only first 3 targets)
        #     Y_eff = Y[:, :args.z0_inner_dim]
        #     loss = loss_func(pred, Y_eff)
            
        # else:
            
        #     # KKThPINN: split loss across (Ca,Cb,Cc) vs (f,g)
        #     # pred: [N,5], Y: [N,5]
        #     pred3  = pred[:, :args.z0_inner_dim]          # [N,3]
        #     predfg = pred[:, args.z0_inner_dim:]          # [N,2]
        #     Y3     = Y[:, :args.z0_inner_dim]
        #     Yfg    = Y[:, args.z0_inner_dim:]

        #     mse3  = loss_func(pred3,  Y3)
        #     msefg = loss_func(predfg, Yfg)

        #     # weights — keep simple constants so we don't touch argparse/config
        #     w3, wfg = 1.0, 0.01   # you can try (1.0, 0.1–0.5)
        #     loss = w3*mse3 + wfg*msefg
    ...
    try:
        ...
        with torch.no_grad():
            for batch_idx, (X, Y) in enumerate(data['test_loader']):
                ...
                print("pred mean/std:", pred.mean(0).tolist(), pred.std(0).tolist())
                print("Y    mean/std:",    Y.mean(0).tolist(),    Y.std(0).tolist())
                print("max|pred| vs max|Y|:", float(pred.abs().max()), float(Y.abs().max()))

                # Also log the exact A,B,b actually used by the model right now:
                print("A_used:", model.A.cpu().numpy())
                print("B_used:", model.B.cpu().numpy())
                print("b_used:", model.b.cpu().numpy())

    # Recompute f,g from the *labels* Ca,Cb,Cc using your existing function
                if args.model == 'KKThPINN':
                    
                    # Recompute h from the *labels* Ca,Cb,Cc to verify the dataset
                    pred4_from_labels = _augment_with_fg_from_nn(X, Y[:, :3], data, args)  # [N,4] -> (Ca,Cb,Cc,h)
                    h_calc  = pred4_from_labels[:, 3:4]   # recomputed h (scaled space)
                    h_label = Y[:, 3:4]                   # h from the dataset (scaled space)
                    diff_mean = torch.abs(h_calc - h_label).mean().item()
                    diff_max  = torch.abs(h_calc - h_label).max().item()
                    print("DEBUG dataset h mismatch (should be ~0) mean:", diff_mean, "max:", diff_max)
                    ...
                    g = (kf) * Ca_u * (Cb_u**2)
                    f = (kr) * (torch.as_tensor(Cao) - Ca_u + torch.as_tensor(Cbo) - Cb_u + torch.as_tensor(Cco))
                    h = (f - g) / (data['scaler'].scale_[4])
                    pre = torch.cat([Ca, Cb, Cc, h], 1)       # pre-projection
