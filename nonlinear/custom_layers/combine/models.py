...
class NNOPT(nn.Module):
    ...
    def forward(self, x):
        ...
        g = (kf/self.scaler.scale_[4]) * Ca_unscaled * (Cb_unscaled ** 2)
        f = (kr/self.scaler.scale_[4]) * (Cao - Ca_unscaled + Cbo - Cb_unscaled + Cco)
        h = (f - g)        
        # inside NNOPT.forward, after computing z0, Ca, Cb, Cc, h
        self._last_inner3 = z0            # [N,3]  (Ca,Cb,Cc) BEFORE projection
        self._last_h = h                  # [N,1]  the single physics channel
        
        basis_outputs = torch.cat([Ca, Cb , Cc, h], dim=1)
