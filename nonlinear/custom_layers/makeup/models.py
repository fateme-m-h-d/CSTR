...
class NNOPT(nn.Module):
    ...
    def forward(self, x):
        ...
        # -----------------------------
        kfCb2 = kf * (Cb_unscaled**2)
        krCa2 = kr * (Ca_unscaled**2)
        
        kfCb2_scaled = kfCb2 / self.scaler.scale_[3]
        krCa2_scaled = krCa2 / self.scaler.scale_[4]
        
        basis_outputs = torch.cat([Ca, Cb , kfCb2_scaled, krCa2_scaled], dim=1)
        ...
        kfCb2_projected = z[:, 2:3] * self.scaler.scale_[3]
        krCa2_projected = z[:, 3:4] * self.scaler.scale_[4]

        # Recover physical Cb and Ca
        Cb_recovered = torch.sqrt(torch.clamp(kfCb2_projected / kf, min=eps))
        Ca_recovered = torch.sqrt(torch.clamp(krCa2_projected / kr, min=eps))
        
        lifted_residual = -tau * kfCb2_projected + tau * krCa2_projected

        recovered_residual = (
            -tau * kf * (Cb_recovered ** 2)
            + tau * kr * (Ca_recovered ** 2)
        )
        ...
        # Final output:
        # [recovered Ca, recovered Cb, projected kfCb2, projected krCa2]
        z = torch.cat(
            [
                Ca_recovered_scaled,
                Cb_recovered_scaled,
