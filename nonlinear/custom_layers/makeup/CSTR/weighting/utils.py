...
def get_violation(args, data, X, pred):
    """
    Nonlinear violation calculated from the final predicted Ca and Cb.

    pred columns:
        pred[:, 0] = Ca
        pred[:, 1] = Cb

    For KKThPINN, pred may have 4 columns:
        [Ca, Cb, kfCb2, krCa2]

    But for violation, we only use final predicted Ca and Cb.
    """
    ...
    violation = -kf * (Ca) * tau + kr * (Cb ** 2) * tau + Cao - Ca
