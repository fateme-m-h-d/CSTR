...
import os
import pandas as pd
from models import Afo, Eaf, Aro, Ear, R, Cao, Cbo, tau
import math
...
def optimizer_step(model, optimizer, loss_func, X, Y, args, data, lambda_k=None, mu_k=None):
    if isinstance(loss_func, PINNLoss):
        ...
        # Use only Ca and Cb for loss calculation
        pred_eff = pred[:, :args.z0_inner_dim]
        # NEW: train NN on only the first z0_inner_dim targets (e.g., Ca,Cb,Cc)
        Y_eff = Y[:, :args.z0_inner_dim]
    
        # # loss = loss_func(pred_eff, Y_eff)
        
        # if getattr(args, "weighted_cb_loss", False):
        #     loss = ca_cb_weighted_loss(pred_eff, Y_eff, data, args)
        # else:
        #     loss = loss_func(pred_eff, Y_eff)
        # loss.backward()
        # optimizer.step()
        ...
        if getattr(args, "weighted_cb_loss", False):
            data_loss = ca_cb_weighted_loss(pred_eff, Y_eff, data, args)
        else:
            data_loss = loss_func(pred_eff, Y_eff)

        z4_penalty = torch.zeros((), device=pred.device, dtype=pred.dtype)

        if getattr(args, "z4_margin_penalty", False):
            z4_penalty = z4_margin_penalty(pred, data, args)

        loss = data_loss + args.beta_z4 * z4_penalty
...
def evaluate_model(data, args):
    ...
    # Recompute f,g from the *labels* Ca,Cb,Cc using your existing function
                ...
                #     pred5_from_labels = _augment_with_lifted_from_nn(X, Y[:, :3], data, args)  # [N,5]
                #     fg_calc  = pred5_from_labels[:, 3:4]        # recomputed f,g (scaled space)
                #     fg_label = Y[:, 3:4]                        # f,g from the dataset (scaled space)

                #     diff_mean = torch.abs(fg_calc - fg_label).mean().item()
                #     diff_max  = torch.abs(fg_calc - fg_label).max().item()
                #     print("DEBUG dataset f,g mismatch (should be ~0) mean:", diff_mean, "max:", diff_max)
                #     mse_per_dim = torch.zeros(Y.shape[1], dtype=torch.float32)
                ...
                if args.model == 'NN':
                    pred_for_violation = _augment_with_lifted_from_nn(X, pred, data, args)  # [N,5]
                else:
                    pred_for_violation = pred
                ...
                T = X[:, 0:1] * torch.as_tensor(sc.scale_[0], device=X.device, dtype=X.dtype)

                Ca = pred_for_violation[:, 0:1] * torch.as_tensor(sc.scale_[1], device=X.device, dtype=X.dtype)
                Cb = pred_for_violation[:, 1:2] * torch.as_tensor(sc.scale_[2], device=X.device, dtype=X.dtype)

                z3 = pred_for_violation[:, 2:3] * torch.as_tensor(sc.scale_[3], device=X.device, dtype=X.dtype)
                z4 = pred_for_violation[:, 3:4] * torch.as_tensor(sc.scale_[4], device=X.device, dtype=X.dtype)

                kf = Afo * torch.exp(-Eaf / (R * T))
                kr = Aro * torch.exp(-Ear / (R * T))

                lifted_residual = -z3 + tau * z4 + Cao

                recovered_residual = -(kf * tau + 1.0) * Ca + tau * kr * (Cb ** 2) + Cao

                z4_from_recovered_cb = kr * (Cb ** 2)
                z4_mismatch = z4_from_recovered_cb - z4

                print("min z4:", z4.min().item())
                print("number z4 < 0:", (z4 < 0).sum().item())
                print("number z4 == 0:", (z4 == 0).sum().item())
                print("number z4 very close to zero:", (z4.abs() < 1e-12).sum().item())

                print("mean |lifted residual|:", lifted_residual.abs().mean().item())
                print("mean |recovered residual|:", recovered_residual.abs().mean().item())

                print("mean |kr*Cb^2 - z4|:", z4_mismatch.abs().mean().item())
                print("max  |kr*Cb^2 - z4|:", z4_mismatch.abs().max().item())
