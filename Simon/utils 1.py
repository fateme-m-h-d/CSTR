...
def get_ScaleAndMean(scaler, x_dim, z_dim):
    ...
    df = pd.DataFrame({"Scaler": scaler, "xscale": xscale, "zscale": zscale})
...
class Data_cstr(data.Dataset):
    ...
    def __init__(self, dataset):
        ...
        self.X = self.dataset_tensor[:, :1] #change this according to the dimension of input
        self.Y = self.dataset_tensor[:, 2:] #change this according to the dimension of output
        self.train_set, self.val_set, self.test_set = self.split_data(0.2)  # initial val_ratio -> 0.2

        #input to be changed
        self.A = torch.tensor([[d], [h]],dtype=torch.float32)  #(2,1)
        
        #output to be changed
        self.B = torch.tensor([[a, c], [f, g]],dtype=torch.float32)  # (2, 2)
        #constant to be changed
        self.b = torch.tensor([e, i],dtype=torch.float32) #(2,1)
