...
def LoadData(args):
    ...
    if args.dataset_type == 'cstr':
        B_dep = B[:, :1]                                       # having a squared dependant matrix, because it is going to be inverse.
        B_indep = B[:, 1:]
    elif args.dataset_type == 'plant':
        B_dep = B[:, :1]                                       # 1 row and 1 column
        B_indep = B[:, 1:]
    elif args.dataset_type == 'distillation':
        B_dep = B[:, :2]                                       # 2 row and 2 column
        B_indep = B[:, 2:]
    else:
        raise ValueError('Dataset not supported!')
...
# def save_scaler_to_csv(scaler):
        # print(scaler)
   
        # # Convert the data to a DataFrame
        # df = pd.DataFrame(scaler)
        # # Save to CSV
        # df.to_csv("./scaler.csv", index=False)
        # print(f"Scaler data saved")
...
class Data_cstr(data.Dataset):
    def __init__(self, dataset):
        ...
        # self.A = torch.tensor([[-2.92570077780229]
        #                         ])  #changed
        # self.B = torch.tensor([[-631.847176731902, -630.847176731902, 0]
        #                         ])  #changed
        # self.b = torch.tensor([-2719.75943931282])    #changed
        
        self.A = torch.tensor([[0]
                                ])  #changed
        self.B = torch.tensor([[1, 1, 1]
                                ])  #changed
        self.b = torch.tensor([3])    #changed
